...
fn min_max_to_html(layers: &[Layer], file_path: &str) -> Result<(), Box<dyn Error>> {
    ...
    html.push_str("  <th>Minimum Area</th>\n");
    html.push_str("  <th>Maximum Area</th>\n");
    html.push_str("</tr>\n");

    for layer in layers {
        let average_area = calculate_average_area(&layer.objects);
        let min_area = calculate_min_area(&layer.objects);
        let max_area = calculate_max_area(&layer.objects);

        html.push_str("<tr>");
        html.push_str(&format!("<td>{}</td>", layer.name));
        html.push_str(&format!("<td>{}</td>", average_area));
        html.push_str(&format!("<td>{}</td>", min_area));
        html.push_str(&format!("<td>{}</td>", max_area));
        html.push_str("</tr>\n");
    }
...
fn calculate_min_area(circles: &[Circle]) -> f64 {
    let mut min_area = std::f64::MAX;
    const PI:f64 = 3.142;

    for circle in circles {
        let area = 2.0 * PI * circle.r.powi(2);
        if area < min_area {
            min_area = area;
        }
    }
...
fn calculate_max_area(circles: &[Circle]) -> f64 {
    let mut max_area = std::f64::MIN;
    const PI:f64 = 3.142;
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    min_max_to_html(&layers, "min_max.html")?;
